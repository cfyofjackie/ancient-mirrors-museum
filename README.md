...
一个偏国风、博物馆体验的互动小工具：经过两页序厅后，滑动切换 **商 → 春秋 → 战国 → 汉 → 隋 → 唐 → 宋 → 元 → 明** 九个时期的代表性铜镜。每面镜先静置于对应时代的平面画卷中，点击后苏醒为可翻转的三维铜镜；镜面保留人物艺术复原，镜背可查看纹样与馆藏史实。为小红书「小工具」（Builder Hub）设计，同时保留扩展为普通网站的能力。
...
- **上下滑动切换页面**（两页序厅 + 九面铜镜，循环），拖动跟手、松手回弹；桌面端支持滚轮和方向键
- **画卷唤醒**：九朝各有横版与手机竖版画卷；第一次点击让二维铜镜原地苏醒为三维镜背
- **点击铜镜翻面**：苏醒后沿 Y 轴翻转，650ms 缓动；WebGL 不可用时回退 CSS 3D
- **纹样热点**：主动点击“细看纹样”后显示，避免与铜镜翻面争抢点击
- **史实资料**：九面镜子均已接入本地馆藏参考图与说明
- **照见古人**：翻到镜面后显示对应朝代的人物意象；人物作为三维镜面材质的一部分随铜镜运动，并明确标注为艺术复原
- 移动端优先：全屏固定布局，纵向手势完全让给朝代切换

## 技术栈

Vite · React 18 · TypeScript · Framer Motion · Three.js · 原生 CSS（无 UI 框架）
...
| 命令 | 用途 |
|---|---|
| `npm run placeholders` | 重新生成早期四镜占位图（历史开发工具，不覆盖当前九镜完整素材） |
| `npm run font:subset` | 霞鹜文楷子集化：扫描源码全部用字，输出 `src/fonts/*.woff2`（新增文案后需重跑） |

## 目录速览

```text
assets/            # 静态资源根（九镜纹理、序厅画作、馆藏参考图）
src/
  data/mirrors.ts  # 全部内容配置：文案、图片路径、热点坐标、史实资料
  data/reflections.ts # 九朝人物旧影配置：素材、标注、显影强度和构图微调
  data/tableaux.ts # 九朝横版/竖版画卷配置
  components/      # MirrorStage / MirrorFlip / Hotspot / InfoCard
  fonts/           # 霞鹜文楷子集（构建期生成，入库）
  textures/tableaux/ # 九朝画卷背景（每朝桌面版 + 手机版）
scripts/           # 占位图 / 字体子集化脚本
docs/screenshots/  # README 展示图
```

铜镜内容全部数据驱动：增加镜子时在 `mirrors.ts` 添加配置并放入对应素材，同时更新序列和完整性检查。
...
九镜扩充后的资源调度、镜面材质和响应式布局修复见 [当前交接文档](HANDOFF.md)。
