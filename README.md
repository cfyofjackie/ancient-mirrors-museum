# 照见千年 · 铜镜小展览

> 从一面铜镜，看见不同时代的审美。

一个偏国风、博物馆体验的互动小工具：经过两页序厅后，滑动切换 **商 → 春秋 → 战国 → 汉 → 隋 → 唐 → 宋 → 元 → 明** 九个时期的代表性铜镜，点击翻面，触摸纹样热点阅读说明，查看馆藏史实资料。为小红书「小工具」（Builder Hub）设计，同时保留扩展为普通网站的能力。

| 移动端 | 馆藏史实资料卡 | 桌面端 |
|---|---|---|
| ![移动端](docs/screenshots/mobile-home.png) | ![史实资料卡](docs/screenshots/mobile-reference.png) | ![桌面端](docs/screenshots/desktop-home.png) |

## 功能

- **上下滑动切换页面**（两页序厅 + 九面铜镜，循环），拖动跟手、松手回弹；桌面端支持滚轮和方向键
- **点击铜镜翻面**：Three.js 沿 Y 轴翻转，650ms 缓动；WebGL 不可用时回退 CSS 3D，默认展示纹样更丰富的镜背
- **纹样热点**：在镜背停留 1.6 秒后轻微浮现，点击弹出底部信息卡
- **史实资料**：九面镜子均已接入本地馆藏参考图与说明
- 移动端优先：全屏固定布局，纵向手势完全让给朝代切换

## 技术栈

Vite · React 18 · TypeScript · Framer Motion · Three.js · 原生 CSS（无 UI 框架）

## 快速开始

```bash
npm install
npm run dev        # http://localhost:6180
npm run build      # 类型检查 + 产物构建（dist/）
npm run preview    # 预览构建产物（http://localhost:6181）
```

## 工程脚本

| 命令 | 用途 |
|---|---|
| `npm run placeholders` | 重新生成早期四镜占位图（历史开发工具，不覆盖当前九镜完整素材） |
| `npm run font:subset` | 霞鹜文楷子集化：扫描源码全部用字，输出 `src/fonts/*.woff2`（新增文案后需重跑） |

## 目录速览

```text
assets/            # 静态资源根（九镜纹理、序厅画作、馆藏参考图）
src/
  data/mirrors.ts  # 全部内容配置：文案、图片路径、热点坐标、史实资料
  components/      # MirrorStage / MirrorFlip / Hotspot / InfoCard
  fonts/           # 霞鹜文楷子集（构建期生成，入库）
scripts/           # 占位图 / 字体子集化脚本
docs/screenshots/  # README 展示图
```

铜镜内容全部数据驱动：增加镜子时在 `mirrors.ts` 添加配置并放入对应素材，同时更新序列和完整性检查。

## 素材与授权

- **馆藏参考图**：来源与说明集中记录在 `assets/references/knowledge.md` 和 `src/data/mirrors.ts`
- **字体**：[霞鹜文楷（LXGW WenKai）](https://github.com/lxgw/LxgwWenkai)，SIL OFL 1.1，子集化后本地打包
- **镜背主视觉**：九镜均使用本地颜色纹理和法线纹理；正面由静态程序纹理表现打磨古铜质感
- 目标平台为小红书小工具沙箱：**零网络请求**，全部资源本地打包

## 性能修复（2026-09-03）

已修复可见内容瞬移、短滑无响应与按钮命中；3D 改为按需绘制，纹理加载去重并补全法线贴图绑定。当前进展与验证见 [修复记录](docs/performance-fix-2026-09-03.md)，原始故障证据见 [诊断报告](docs/performance-diagnosis-2026-09-03.md)。用户本地手机体验验收通过，已部署到 [GitHub Pages 线上展览](https://cfyofjackie.github.io/ancient-mirrors-museum/)。

九镜扩充后的资源调度、镜面材质和响应式布局修复见 [当前交接文档](HANDOFF.md)。

## 开发进度

设计文档见 [SLC.md](SLC.md)，决策与进度记录见 [PROGRESS.md](PROGRESS.md)。
