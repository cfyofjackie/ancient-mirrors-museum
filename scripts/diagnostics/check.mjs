...
const exactVariant = new RegExp(`^${escapedVariant}-\\d+x\\d+-\\d+\\.json$`)
const files = (await fs.readdir(directory)).filter(f=>exactVariant.test(f)).sort((a,b)=>Number(a.match(/-(\d+)\.json$/)[1])-Number(b.match(/-(\d+)\.json$/)[1]))
...
if (!variant.startsWith('interactions') && (result.visibility !== 'visible' || result.summary.completedSwitches < 8)) throw new Error('Inconclusive: a visible, complete eight-switch run is required')
