...
1. `.page` 只保留位移，不再对包含透明 WebGL canvas 的共同祖先动画 `opacity`；退出/进入的遮蔽改由独立纯色 `.page-transition-mask` 完成，仍保证完全遮住后才交换素材。
2. `pointermove` 不再每个原始事件都写入 MotionValue，而是用 `requestAnimationFrame` 合并，同一帧只提交最后一个手指位置。
